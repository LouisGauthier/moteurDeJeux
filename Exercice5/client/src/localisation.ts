...
export class Localisation {
  ...
  public static get(key: string, queryContext: ILocaleContext = {}) {
    ...
    var resultStr = localeStrings[key];
    for (const k in queryContext) {
      // Si une variable du contexte est contenue dans la chaine, on la remplace par sa valeur
      if (resultStr.includes("{"+k+"}")) {
        resultStr = resultStr.replace("{"+k+"}", queryContext[k]);
      }
    }
